...
colwidths = [1,1,1,0.28]
# create figure
fig, axs = plt.subplots(numrows, numcols, figsize=(7, 13), height_ratios=rowheights,width_ratios=colwidths)
...
for i,node in enumerate(allprecip):
    ...
    #add subplot
    ax = fig.add_subplot(numrows,numcols,locations[i]) 
...
for i, arr in enumerate(som_composites):
    ...
    xi, yi = map(lon,lat)
    ax = fig.add_subplot(numrows,numcols,locations[i]+20)
    if i == 0:
...
cbar_ax = fig.add_axes([0.913,0.015,0.025,0.47]) #[xloc, yloc, xwidth, yheight]
...
fig.subplots_adjust(left=0.07,right=0.99,bottom=0.014, top=0.995,hspace=0.05, wspace=0.05) #bottom colorbar
...
os.chdir(save_dir)
plt.savefig(f'PrecipHistograms_and_SpatialPrecipPatterns_{clusters}d_{numpatterns}node',dpi=300)
plt.show()
