...
for i, arr in enumerate(denordata):
    ...
    for j in range(len(merrareduced)):
        ...
        if i == 0:
            ax.set_title(f'Day {place-5}',fontsize=fontscale+5,fontweight="bold",pad=1)  
        ...
        if place == 5:
            ...
            ax.text(x=xloc, y=0.99, s=f'{perc_assigned}%', transform=ax.transAxes + sublabel_loc,
                fontsize=fontscale+4, fontweight='bold',verticalalignment='top', color = 'k',
                bbox=dict(facecolor=patfreq_col, edgecolor='none', pad=1.5),zorder=3)
        if place == 1:
            # ax.set_title(f'{i+1}',fontsize=12,fontweight="bold",pad=2)        
            ax.set_ylabel(f'{i+1}',fontsize=fontscale+4.5,fontweight="bold",labelpad=10,rotation=0)
        ...
        #add yuba shape
        plt.scatter(-120.9,39.5,color='w',marker='*',linewidths=0.5, s = 100, zorder=4, edgecolors='0.2')
        place +=1
...
fig.subplots_adjust(left=0.035,right=0.96,bottom=0.046, top=0.9908,hspace=0.05, wspace=0.05) #bottom colorbar
#fig.add_axis([left,bottom, width,height])
...
cbar.ax.tick_params(labelsize=fontscale+3)
cbar.set_label(cbarlabs,fontsize=fontscale+4,labelpad=0.5,fontweight='bold')

    
#SHOW MAP
save_dir='I:\\Emma\\FIROWatersheds\\Figures\\SOMs'
os.chdir(save_dir)
plt.savefig(f'{metvar}_{percentile}_{numpatterns}SOM_{clusters}d_testing.png',dpi=300)
plt.show()