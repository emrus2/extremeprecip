...
#%% IMPORT MERRA2 DATA
# define metvar
metvars = ['SLP', '300W','Z500Anom','SLPAnom','Z850','850T']
metvars = ['IVT']
#metvar = '300W'
for metvar in metvars:
    ...
    latmin, latmax = (34.5,43.5)
    lonmin, lonmax = (-131.0,-119.25)
...
#%% INCLUDE CALCULATION OF IVT VECTORS
    ...
    #create subplot for mapping multiple timesteps
        ...
        if metvar == 'Z500Anom':
            ...
            newmap = center_colormap(lowanom, highanom, center=0)
        lowlims = {'Z500':2850,'SLP':985,'IVT':0,'300W':0,'850T':269,'Z500Anom':lowanom,'Z850':1187,'SLPAnom':lowanom}
        highlims = {'Z500':5700,'SLP':1022,'IVT':736,'300W':56,'850T':282,'Z500Anom':highanom,'Z850':1548,'SLPAnom':highanom}
        
        contourstart = {'Z500':3000,'SLP':990,'IVT':0,'300W':5,'850T':270,'Z500Anom':-1.75,'Z850':1190,'SLPAnom':-2.25}
        contourint = {'Z500':200,'SLP':4,'IVT':75,'300W':5,'850T':1,'Z500Anom':0.25,'Z850':30,'SLPAnom':0.25}
        
        cbarstart = {'Z500':3000,'SLP':990,'IVT':0,'300W':0,'850T':270,'Z500Anom':-2.0,'Z850':1200,'SLPAnom':-2.4}
        cbarint = {'Z500':500,'SLP':5,'IVT':100,'300W':10,'850T':2,'Z500Anom':0.5,'Z850':50,'SLPAnom':0.4}
    ...
    for i, arr in enumerate(som_composites):
        ...
        gridlinefont = 8.5
        parallels = np.arange(35.,45.,4.)
        meridians = np.arange(-129.,-119.,4.)
        if i == 0 or i == 3:
    ...
    os.chdir(save_dir)
    plt.savefig(f'{metvar}_{percentile}_{numpatterns}_SOM_composite_Zoom.png',dpi=300)
    plt.show()