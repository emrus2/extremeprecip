...
for i in range(296,297):
    ...
    #map.drawparallels(np.arange(37.,43.,1.), labels=[0,1,0,0], fontsize=10,color=border_c, linewidth=border_w)
    ...
    #create colorbar
    #cbar = map.colorbar(colorm, location='left', pad="15%",ticks=np.arange(lowlim,highlim+1,10))
    
    #show map
    save_dir = f'I:\\Emma\\FIROWatersheds\\Figures\\{watershed}'
    save_file = f'1_{i}_{watershed}_precip.png'
