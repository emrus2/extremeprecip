...
folder = 'I:\\CCAR-2022\\'
file = 'globalARcatalog_MERRA2_1980-2020_v3.0.nc'
...
dates = np.array([datetime.strftime(datetime(1980,1,1)+timedelta(seconds=time[n]),"%Y%m%d") for n in range(time.shape[0])])
...
#%% IMPORT SHAPE DATA
shape = np.squeeze(ds.variables['kidmap'][:,dateind,:,:,:])
...
np.save(os.path.join(savefolder,'ARPresence_90_Percentile_Days_kidmap.npy'),dates_arpresence)
