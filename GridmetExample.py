...
"""
#%%
# import package to use .nc file
import netCDF4 as nc # needs to be installed before importing
...
print(gridfile.variables)
    # we can see the precip shape is 366(days) x 585(deg lat) x 1386(deg lon)

# here is where we can define our variables
gridlat = gridfile.variables['lat'][:]
gridlon = gridfile.variables['lon'][:]
gridday = gridfile.variables['day'][:]

# define precipitation, this will gives us an array of all the precipitaiton data in file
precip = gridfile.variables['precipitation_amount'][:,:,:] #time x height x lat x lon

# let's say I just want the precip of Jan 1
janone = precip[0,:,:]

# let's say I want the average temperature for the entire year
yravg = np.mean(precip,axis=0)

# let's say I want to look temperature for a specific region
