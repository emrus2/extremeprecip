...
cbar = fig.colorbar(colorm, cax=cbar_ax,ticks=np.arange(2,highlim+1,2),orientation='vertical')
cbar.ax.tick_params(labelsize=8)
cbar.set_label('Proportion of Average',fontsize=8.5,labelpad=2,fontweight='bold')
...
if lettered == True:
    if IVT == True:
        plt.savefig(f'GRIMET_PrecipitiationWetSeasonProportion_IVT_{numpatterns}SOM_composite_{clusters}d_lettered.png',dpi=300)
    else:
        plt.savefig(f'GRIMET_PrecipitiationWetSeasonProportion_{numpatterns}SOM_composite_{clusters}d_lettered.png',dpi=300)
else:
    if IVT == True:
        plt.savefig(f'GRIMET_PrecipitiationWetSeasonProportion_IVT_{numpatterns}SOM_composite_{clusters}d.png',dpi=300)
    else:
        plt.savefig(f'GRIMET_PrecipitiationWetSeasonProportion_{numpatterns}SOM_composite_{clusters}d.png',dpi=300)
plt.show()