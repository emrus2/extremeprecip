...
lowlims, highlims = (0,875)
contourstart, contourint = (0,100)
cbarstart, cbarint = (0,125)
colormap = 'gnuplot2_r'
cbarlabs = 'kg/m/s'
plottitle = metvar
...
for i, arr in enumerate(denordata):
    ...
    precipavgnorm = 0.82-(((precipavg-min(precip_rounded))/(max(precip_rounded)+7-min(precip_rounded))))
    ...
    for j in range(len(merrareduced)):
        ...
        xi, yi = map(lon,lat)
        plotloc = 1 + (clusters*i) + j
        ax = fig.add_subplot(numpatterns,clusters,plotloc)
        if i == 0:
            ax.set_title(f'Day {place-3}',fontsize=10,fontweight="bold",pad=1)  
        ...
        if place == 3:
            ...
            ax.text(x=0.38, y=1.0, s='{:.0f}%'.format(arsomfreq_rounded[i]), transform=ax.transAxes + sublabel_loc,
                fontsize=9, fontweight='bold',verticalalignment='top', color = 'k',
...
fig.subplots_adjust(left=0.05,right=0.94,bottom=0.046, top=0.991,hspace=0.05, wspace=0.05) #bottom colorbar
